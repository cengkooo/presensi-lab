    ...
    <span
      ...
      style={
        active
          ...
          : { background: "rgba(255,255,255,0.04)", color: "rgba(134,239,172,0.4)", border: "1px solid rgba(255,255,255,0.06)" }
      }
    >
      {active && (
    ...
    >
      <div
        ...
        style={{
          ...
          boxShadow: "0 0 60px rgba(34,197,94,0.1), 0 24px 80px rgba(0,0,0,0.7)",
        }}
      >
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: "rgba(134,239,172,0.5)" }}>Judul Sesi *</label>
            <input className="input-glass" placeholder="cth: Praktikum Jaringan - A1" value={form.title} onChange={(e) => set("title", e.target.value)} />
          </div>
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: "rgba(134,239,172,0.5)" }}>Deskripsi</label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            <div>
              <label className="block text-xs font-semibold mb-1.5" style={{ color: "rgba(134,239,172,0.5)" }}>Tanggal *</label>
              <input type="date" className="input-glass" value={form.date} onChange={(e) => set("date", e.target.value)} />
            </div>
            <div>
              <label className="block text-xs font-semibold mb-1.5" style={{ color: "rgba(134,239,172,0.5)" }}>Durasi (menit)</label>
              <input type="number" className="input-glass" min={15} max={240} value={form.duration} onChange={(e) => set("duration", Number(e.target.value))} />
            </div>
          </div>
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: "rgba(134,239,172,0.5)" }}>Lokasi *</label>
          ...
          <div>
            <label className="block text-xs font-semibold mb-1.5" style={{ color: "rgba(134,239,172,0.5)" }}>Radius GPS (meter)</label>
    ...
    <div style={{ minHeight: "100vh", padding: "28px 32px 48px", boxSizing: "border-box" }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "28px", gap: "12px" }}>
        <div>
          ...
          <p style={{ color: "rgba(134,239,172,0.5)", fontSize: "14px", marginTop: "6px" }}>Kelola semua sesi praktikum laboratorium</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "16px", marginBottom: "24px" }}>
        {[
          { label: "Total Sesi", value: stats.total, icon: <Calendar size={16} style={{ color: "#4ade80" }} /> },
          { label: "Sesi Aktif", value: stats.active, icon: <Radio size={16} style={{ color: "#f87171" }} /> },
          { label: "Total Hadir", value: stats.totalAttendees, icon: <Users size={16} style={{ color: "#4ade80" }} /> },
        ].map((c) => (
          ...
          {(["all", "active", "done"] as const).map((f) => (
            <button key={f} onClick={() => setFilterStatus(f)}
              style={{
                ...
                border: filterStatus === f ? "1px solid rgba(34,197,94,0.4)" : "1px solid rgba(34,197,94,0.12)",
                background: filterStatus === f ? "rgba(34,197,94,0.12)" : "transparent",
                color: filterStatus === f ? "#4ade80" : "rgba(134,239,172,0.4)",
                transition: "all 0.2s",
              }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "16px" }}>
        {filtered.map((sess) => (
          <div key={sess.id} className="glass glass-hover rounded-2xl"
            ...
            {/* Card Header */}
            <div style={{ padding: "18px 18px 14px", borderBottom: "1px solid rgba(34,197,94,0.08)" }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "8px", marginBottom: "10px" }}>
                ...
                <div style={{ display: "flex", gap: "4px" }}>
                  ...
                  <button onClick={() => handleDelete(sess.id)}
                    ...
                    style={{ color: "rgba(134,239,172,0.4)", transition: "all 0.2s" }}>
                    <Trash2 size={13} />
                  </button>
                </div>
              </div>
              ...
              {sess.description && (
                <p style={{ fontSize: "12px", color: "rgba(134,239,172,0.4)", lineHeight: 1.5 }}>{sess.description}</p>
              )}
            </div>
            {/* Card Body */}
            <div style={{ padding: "14px 18px", display: "flex", flexDirection: "column", gap: "8px", flex: 1 }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <Calendar size={13} style={{ color: "#22c55e", flexShrink: 0 }} />
                <span style={{ fontSize: "13px", color: "rgba(134,239,172,0.65)" }}>{sess.date}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <MapPin size={13} style={{ color: "#22c55e", flexShrink: 0 }} />
                <span style={{ fontSize: "13px", color: "rgba(134,239,172,0.65)" }}>{sess.location}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                <Clock size={13} style={{ color: "#22c55e", flexShrink: 0 }} />
                <span style={{ fontSize: "13px", color: "rgba(134,239,172,0.65)" }}>{sess.duration} menit</span>
                <span style={{ marginLeft: "auto", fontSize: "12px", color: "rgba(134,239,172,0.35)" }}>R: {sess.radius}m</span>
              </div>
            </div>
            {/* Card Footer */}
            <div style={{ padding: "12px 18px", borderTop: "1px solid rgba(34,197,94,0.08)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                <Users size={13} style={{ color: "#4ade80" }} />
                <span style={{ fontSize: "13px", fontWeight: 600, color: "#4ade80" }}>{sess.attendeeCount}</span>
                <span style={{ fontSize: "12px", color: "rgba(134,239,172,0.4)" }}>mahasiswa hadir</span>
              </div>
              {sess.isActive && (
                <span style={{ fontSize: "11px", fontWeight: 600, color: "#4ade80", background: "rgba(34,197,94,0.1)", padding: "3px 8px", borderRadius: 6 }}>
                  LIVE
                </span>
              )}
            </div>
        ...
        {filtered.length === 0 && (
          <div style={{ gridColumn: "1/-1", padding: "48px", textAlign: "center", color: "rgba(134,239,172,0.3)" }}>
