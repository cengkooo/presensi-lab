...
function DistanceBadge({ meters }: { meters: number | null }) {
  ...
  const color = meters <= 50 ? "#34D399" : meters <= 100 ? "#facc15" : "#f87171";
  return (
  ...
  if (loading) {
    return (
      <div style={{ minHeight: "100vh", padding: "28px 32px", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <p style={{ color: "rgba(110,231,183,0.5)", fontSize: "14px" }}>Memuat data sesi...</p>
      </div>
    );
  }
  ...
  if (error || !session) {
    return (
      <div style={{ minHeight: "100vh", padding: "28px 32px" }}>
        <Link href={`/dashboard/kelas/${classId}`} style={{ display: "inline-flex", alignItems: "center", gap: "6px", fontSize: "13px", color: "rgba(110,231,183,0.5)", textDecoration: "none", marginBottom: "24px" }}>
          <ChevronLeft size={14} /> Kembali ke Kelas
        </Link>
        <p style={{ color: "#f87171" }}>{error ?? "Sesi tidak ditemukan."}</p>
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "28px 32px 48px", boxSizing: "border-box" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "24px", flexWrap: "wrap" }}>
        <Link href="/dashboard/kelas" style={{ fontSize: "13px", color: "rgba(110,231,183,0.45)", textDecoration: "none" }}
          onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.color = "#34D399")}
          onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.color = "rgba(110,231,183,0.45)")}>
          Kelas
        </Link>
        <span style={{ color: "rgba(110,231,183,0.2)" }}>/</span>
        <Link href={`/dashboard/kelas/${classId}`} style={{ fontSize: "13px", color: "rgba(110,231,183,0.45)", textDecoration: "none" }}
          onMouseEnter={(e) => ((e.currentTarget as HTMLElement).style.color = "#34D399")}
          onMouseLeave={(e) => ((e.currentTarget as HTMLElement).style.color = "rgba(110,231,183,0.45)")}>
          {cls?.code ?? classId}
        </Link>
        <span style={{ color: "rgba(110,231,183,0.2)" }}>/</span>
        <span style={{ fontSize: "13px", color: "#f0fdf4" }}>{session.title}</span>
      </div>
      ...
      <div className="glass rounded-2xl" style={{ padding: "22px 26px", marginBottom: "20px" }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "16px", flexWrap: "wrap", marginBottom: "16px" }}>
          <div>
            <div style={{ display: "flex", alignItems: "center", gap: "10px", flexWrap: "wrap", marginBottom: "6px" }}>
              <h1 style={{ fontSize: "20px", fontWeight: 700, color: "#f0fdf4" }}>{session.title}</h1>
              {session.is_active ? (
                ...
                <span style={{ fontSize: "11px", fontWeight: 600, padding: "2px 9px", borderRadius: "20px", background: "rgba(16,185,129,0.1)", color: "#34D399", border: "1px solid rgba(16,185,129,0.2)" }}>
                  Selesai
                </span>
              )}
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: "14px", flexWrap: "wrap" }}>
              <span style={{ display: "flex", alignItems: "center", gap: "4px", fontSize: "12px", color: "rgba(110,231,183,0.5)" }}>
          ...
          <button onClick={fetchData} style={{ padding: "8px 12px", borderRadius: "10px", background: "transparent", border: "1px solid rgba(16,185,129,0.2)", color: "rgba(110,231,183,0.5)", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", flexShrink: 0 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "10px" }}>
          {[
            { label: "Total", value: total, icon: <Users size={13} style={{ color: "#34D399" }} />, color: "#f0fdf4" },
            { label: "Hadir", value: hadir, icon: <CheckCircle size={13} style={{ color: "#34D399" }} />, color: "#34D399" },
            { label: "Absen", value: absen, icon: <XCircle size={13} style={{ color: "rgba(110,231,183,0.4)" }} />, color: "rgba(110,231,183,0.7)" },
            { label: "Ditolak", value: ditolak, icon: <AlertTriangle size={13} style={{ color: "#f87171" }} />, color: "#f87171" },
          ].map((s) => (
            <div key={s.label} style={{ padding: "10px 12px", borderRadius: "10px", background: "rgba(16,185,129,0.04)", border: "1px solid rgba(16,185,129,0.1)" }}>
      ...
      <div className="glass rounded-2xl" style={{ overflow: "hidden" }}>
        <div style={{ padding: "14px 20px", borderBottom: "1px solid rgba(16,185,129,0.1)" }}>
        ...
        {total === 0 ? (
          <div style={{ padding: "40px 24px", textAlign: "center" }}>
            ...
            <p style={{ color: "rgba(110,231,183,0.4)", fontSize: "14px" }}>Belum ada data absensi untuk sesi ini.</p>
          </div>
        ) : (
          <div>
            {attendances.map((att) => {
              ...
              return (
                ...
                >
                  <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg,#1e3a1e,#2d5a2d)", border: "1px solid rgba(16,185,129,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "10px", fontWeight: 700, color: "#34D399", flexShrink: 0 }}>
                    {getInitial(prof?.full_name ?? null)}
                  </div>
                  <div style={{ flex: 1 }}>
                    <p style={{ fontSize: "13px", fontWeight: 600, color: "#f0fdf4" }}>{prof?.full_name ?? "—"}</p>
                    <p style={{ fontSize: "11px", color: "rgba(110,231,183,0.4)" }}>NIM {prof?.nim ?? "—"}</p>
                  </div>
                  <DistanceBadge meters={att.distance_meter} />
                  <div style={{ textAlign: "right" }}>
                    {att.checked_in_at && (
                      <p style={{ fontSize: "11px", color: "rgba(110,231,183,0.4)", marginBottom: "3px" }}>
