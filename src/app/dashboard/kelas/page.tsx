...
function Field({
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
      <label style={{ fontSize: 11, color: "rgba(134,239,172,0.6)", fontWeight: 600, letterSpacing: "0.05em", textTransform: "uppercase" }}>
      ...
      />
      {hint && <p style={{ fontSize: 11, color: "rgba(134,239,172,0.4)", marginTop: 2 }}>{hint}</p>}
    </div>
  );
}
    ...
    <div style={{ minHeight: "100vh", padding: "28px 32px 48px", boxSizing: "border-box" }}>
      ...
      <div style={{ marginBottom: "28px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
        ...
        <button
          ...
          onMouseEnter={(e) => (e.currentTarget.style.background = "linear-gradient(135deg,rgba(22,163,74,0.4),rgba(34,197,94,0.25))")}
          onMouseLeave={(e) => (e.currentTarget.style.background = "linear-gradient(135deg,rgba(22,163,74,0.25),rgba(34,197,94,0.15))")}
        >
          <Plus size={16} />
          Buat Kelas Baru
        </button>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
        {classes.length === 0 && (
          <div className="glass rounded-2xl" style={{ padding: "48px 24px", textAlign: "center" }}>
            ...
            <p style={{ color: "rgba(134,239,172,0.5)", fontSize: 15 }}>Belum ada kelas. Klik <strong style={{ color: "#4ade80" }}>Buat Kelas Baru</strong> untuk memulai.</p>
          </div>
        )}
        ...
        <div style={{
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 36, height: 36, borderRadius: 10, background: "rgba(34,197,94,0.12)", border: "1px solid rgba(34,197,94,0.25)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <FlaskConical size={18} style={{ color: "#4ade80" }} />
                </div>
                <div>
                  <h2 style={{ color: "#f0fdf4", fontSize: 17, fontWeight: 700, margin: 0 }}>Buat Kelas Baru</h2>
                  <p style={{ color: "rgba(134,239,172,0.45)", fontSize: 12, margin: 0 }}>Isi detail kelas praktikum</p>
                </div>
              </div>
              <button onClick={() => setShowModal(false)}
                style={{ background: "none", border: "none", cursor: "pointer", color: "rgba(134,239,172,0.5)", lineHeight: 1, padding: 4, borderRadius: 6 }}>
                <X size={20} />
              </button>
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              {/* Formula preview */}
              <div style={{ background: "rgba(34,197,94,0.06)", border: "1px solid rgba(34,197,94,0.15)", borderRadius: 10, padding: "10px 14px", display: "flex", alignItems: "center", gap: 10 }}>
                <CheckCircle size={14} style={{ color: "#4ade80", flexShrink: 0 }} />
                <p style={{ fontSize: 12, color: "rgba(134,239,172,0.7)", margin: 0 }}>
                  % Kehadiran = <strong style={{ color: "#4ade80" }}>Sesi Dihadiri / {form.total_sessions_planned || "?"} × 100</strong>
                  {" "}· Lulus jika ≥ <strong style={{ color: "#4ade80" }}>{form.min_attendance_pct || "?"}%</strong>
                </p>
              </div>
            </div>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 22, justifyContent: "flex-end" }}>
              <button onClick={() => setShowModal(false)}
                style={{ padding: "9px 18px", borderRadius: 10, background: "none", border: "1px solid rgba(134,239,172,0.2)", color: "rgba(134,239,172,0.6)", fontSize: 13, cursor: "pointer" }}>
                Batal
              </button>
              <button onClick={handleSubmit}
                style={{ padding: "9px 22px", borderRadius: 10, background: "linear-gradient(135deg,rgba(22,163,74,0.35),rgba(34,197,94,0.2))", border: "1px solid rgba(34,197,94,0.35)", color: "#4ade80", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
      ...
      <style>{`
        ...
        input:focus { border-color: rgba(34,197,94,0.5) !important; box-shadow: 0 0 0 2px rgba(34,197,94,0.08); }
      `}</style>
    </div>
  );
}
