...
export default function PresensiPage() {
  ...
  const handleGoogleLogin = useCallback(async () => {
    ...
    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/api/auth/callback?next=/presensi`,
      },
    });
  }, []);
      ...
      >
        ...
        <div className="relative z-10 w-full max-w-sm space-y-4 animate-fade-slide-up">
          <GlassCard variant="strong" className="p-8 text-center">
            <Logo />
            <p className="text-sm mt-4 mb-6" style={{ color: "var(--text-muted)" }}>
              Login untuk mencatat kehadiran praktikum kamu.
            </p>
            <GlassButton
              variant="ghost"
              fullWidth
